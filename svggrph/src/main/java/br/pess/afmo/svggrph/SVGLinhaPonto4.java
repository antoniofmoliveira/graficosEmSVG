 ...
 */
public class SVGLinhaPonto4 extends GraficoSVGAbstrato {
...
/**
    ...
    public Document getSVGDocument() throws UnsupportedEncodingException, TranscoderException {
        ...
        for (int i = 0; i < tics; i++) {
            ...
            etext.appendChild(text("text", "x:468;y:" + (y + 3), "font-size:7px;text-anchor:start;",
                    nfi.format((yMaxRight - (d2 * i)))));
        }

        etext.appendChild(text("text", "x:468;y:240", "font-size:8px;text-anchor:end;", "0"));
                ...
                for (int j = 0; j < seriesLinha[i].length; j++) {
                    if (seriesLinha[i][j] > 0) {
                        elinhas.appendChild(create("circle",
                                "r:3;stroke:black;cx:" + (nx[j] - (n * .9) + (i * larguraColuna) + (larguraColuna / 2)) //(mx[j]-o+i*larguraColuna)
        ...
        for (int i = 0; i < seriesColuna.length; i++) {
            fimAnt = i;
            etext.appendChild(text("text", "x:22;y:" + (255 + (10 * i)), "font-size:6px;text-anchor:start;", seriesNamesColuna[i]));

            for (int j = 0; j < seriesColuna[i].length; j++) {
//                Element te = text("text",
//                        "x:" + mx[j] + ";y:" + (250 + (10 * i)) + ";transform:rotate(" + anguloValorTabela[i] + "," + mx[j] +
//                        "," + (250 + (10 * i)) + ")",
//                        "font-size:6px;text-anchor:" + ((anguloValorTabela[i] != 0) ? "end" : "middle"), null);
//                te.appendChild(text("tspan", "dy:+.5em", null, nfsColuna[i].format(seriesColuna[i][j])));
//                etext.appendChild(te);
            }

            if (plotar[i]) {
                ecolunas.appendChild(create("rect",
                        "stroke-width:0.1;stroke:black;width:8;height:8;x:6;y:" + (248 + (10 * i)) + ";fill:" + colors[i]));
            }
        }

//        fimAnt += .5;
        //egraf.appendChild(create("line","x1:80;y1:"+(255 + (10 * fimAnt))+ ";x2:460;y2:"+(255 + (10 * fimAnt)) ));
//        fimAnt += 1;
        ...
        for (int i = 0; i < seriesLinha.length; i++) {
            ...
            etext.appendChild(text("text", "x:22;y:" + (255 + (10 * (i + fimAnt))), "font-size:6px;text-anchor:start;", seriesNamesLinha[i]));

            
            for (int j = 0; j < seriesLinha[i].length; j++) {
//                Element te = text("text",
//                        "x:" + mx[j] + ";y:" + (250 + (10 * (i + fimAnt))) + ";transform:rotate(" + anguloValorTabela[i] + "," +
//                        mx[j] + "," + (250 + (10 * (i + fimAnt))) + ")",
//                        "font-size:6px;text-anchor:" + ((anguloValorTabela[i] != 0) ? "end" : "middle"), null);
//                te.appendChild(text("tspan", "dy:+.5em", null, nfsColuna[i].format(seriesLinha[i][j])));
//                etext.appendChild(te);
                //                etext.appendChild(text("text", "x:" + mx[j] + ";y:" + (255 + (10 * (i + fimAnt )))+";transform:rotate(-90," + mx[j] + ","+(255 + (10 * (i + fimAnt )))+")",
                //                        "font-size:6px;text-anchor:end;", nfsLinha[i].format(seriesLinha[i][j])));
            }

            if (plotar[i]) {
                //                elinhas.appendChild(create("path",
                //                        "stroke:" + colors[i] + ";stroke-width:3;fill:" + colors[i] + ";d:" + "M 2" +
                //                        s(297 + (10 * (i + fimAnt + 1))) + " L 18" + s(297 + (10 * (i + fimAnt + 1)))));
                elinhas.appendChild(create("circle",
                        "stroke-width:0.1;stroke:black;r:3;cx:10;cy:" + (252 + (10 * (i + fimAnt))) + ";fill:" + colors[i]));
            }
...
//        }
        ...
        for (int i = 0; i < tags.length; i++) {
            Element te = text("text",
                    "x:" + mx[i] + ";y:" + novoY + ";transform:rotate(" + anguloTag + "," + mx[i] + "," + novoY + ")",
                    "font-size:6px;text-anchor:" + ((anguloTag != 0) ? "end" : "middle"), null);
            te.appendChild(text("tspan", "dy:+.5em", null, tags[i]));
            etext.appendChild(te);

            //            etext.appendChild(text("text", "x:" + mx[i] + ";y:"+novoY+";transform:rotate(-90," + mx[i] + ","+novoY+")", "font-size:6px;text-anchor:end;", tags[i]));
            //            etext.appendChild(text("text", "x:" + mx[i] + ";y:"+novoY, "font-size:6px;text-anchor:middle;", tags[i]));
        }
        ...
        etext.appendChild(text("text", "id:tituloyright;x:500;y:140;transform:rotate(-90,500,140)",
                "font-size:12;text-anchor:middle;", yTituloRight));

        Element t = text("text",
                "id:base;x:" + (size * 1.03) + ";fill:black;y:" +
                ((size * .6) + (((seriesColuna.length * 2) > 8) ? ((((seriesColuna.length * 2) - 8) * 7) + 20) : 0)), //(((size * .7)+(seriesColuna.length*2>8?((seriesColuna.length*2-8)*20):0))),
                "text-anchor:end;font-size:6", null);
        t.appendChild(text("tspan", "dx:-4em;dy:-1em", null, base));
        etext.appendChild(t);

        if (observacoes.length > 0) {
            t = text("text",
                    "id:base;x:5;fill:black;y:" +
                    (((size * .6) + (((seriesColuna.length * 2) > 8) ? ((((seriesColuna.length * 2) - 8) * 7) + 20) : 0)) -
                    (7 * observacoes.length)), "text-anchor:start;font-size:6", null);

            for (int i = 0; i < observacoes.length; i++) {
                t.appendChild(text("tspan", "x:5;dy:0em", null, observacoes[i]));
            }
    ...
    protected Element prepare() {
        ...
        setAttributes(svgRoot,
            "width:" + (largura * 1.03) + ";height:" +
            ((largura * .6) + (((seriesColuna.length * 2) > 8) ? ((((seriesColuna.length * 2) - 8) * 20) + 20) : 0)));
